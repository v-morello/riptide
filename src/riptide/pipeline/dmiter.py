...
class DMIterator(object):
    ...
    def __init__(
        ...
        log.info(
            f"Selecting DM trials in the range {self.dm_start:.4f} to {self.dm_end:.4f}"
        )
        ...
        try:
            ...
            log.info(
                f"Could not infer observing band parameters from input files: {err!s}"
            )
