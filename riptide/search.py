...
def ffa_search(tseries, rmed_width=4.0, rmed_minpts=101, period_min=1.0, period_max=30.0, fpmin=8, bins_min=240, bins_max=260, ducy_max=0.20, wtsp=1.5, threads=1):
    ...
    period_min: float
        Minimum period to search in seconds (default: 1.0)
    period_max: float
        Maximum period to search in seconds (default: 30.0)
    fpmin: int
    ...
    bins_max: int
        ...
        larger than bins_min, preferably > 10% larger to avoid downsampling
        the input data too many times.
        (default: 260)
    wtsp: float
